<template>
  <nav id="navbar">
    <v-app-bar flat app dense>
      ...
      <v-btn text color="grey" v-on:click="signOut" v-show="loginState">
        <span class="hidden-sm-and-down">Sign Out</span>
        <v-icon right>mdi-logout</v-icon>
      </v-btn>

      <!-- <v-btn text color="grey" v-on:click="exitApp" v-show="isServer">
        <span class="hidden-sm-and-down">Shutdown</span>
        <v-icon right>mdi-close-network-outline</v-icon>
      </v-btn> -->
...
export default {
  ...
  computed: {
    ...
    //   return currentUrl.hostname === "127.0.0.1"
    // }
  },
  watch: {
    dialog: function (val) {
      if (val && !this.qrcodeState) {
        ...
        if (curr_url.hostname === "127.0.0.1") {
          this.serverUrl =
            ...
            ":" +
            curr_url.port;
        } else this.serverUrl = curr_url.protocol + "//" + curr_url.host;
